          ...
          try {
            ...
            if (!hasSubmitAttempt(currentPart)) {
              ...
              if (success) {
                if (currentPart === "2") {
                  notify(
                    `✅⭐️⭐️ Part 2 Completed! You rock! You may safely resume your life now`
                  );
                  resolve();
                } else {
                  resetTestsOutputs();
                  backupPart1Solution();
                  notify(
                    `✅⭐️ Part 1 Completed! Tests reset and solution backed-up. Opening browser! Good luck on next part!`
                  );
                  currentPart = "2";
                ...
                notify(
                  `😞❌ Nope, not it. Doesn't look right. Auto submission will stop working. To revive it delete relevant submission file`
                );
              }
            } else {
              notify(
            ...
            notify(`Error running solution on input / submitting it - ${e}`);
          }
        } else {
