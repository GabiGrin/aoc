...
import { fastMax, fastMin } from "./math";
...
export const createGrid = <T>(inner: Map<string, T> = new Map()): Grid<T> => {
    ...
    const grid: Grid<T> =  {
        ...
        width: () => {
            ...
            const min = fastMin(xs);
            const max = fastMax(xs);
            return (max - min) + 1
        },
        height: () => {
            const ys = keys(inner).map(k => fromId(k).y);
            const min = fastMin(ys);
            const max = fastMax(ys);
            return (max - min) + 1;
        },
        reduce: (fn, init) => {
        ...
        toString: () => {
            ...
            const fromX = fastMin(xs);
            const toX = fastMax(xs);

            const fromY = fastMin(ys);
            const toY = fastMax(ys);
